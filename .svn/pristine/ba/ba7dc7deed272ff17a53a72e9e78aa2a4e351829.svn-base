...
<title>회원관리 사용자 로그인</title>
<script type="text/javascript">
	$(function() {
		$('img[class=loginBtn]').click(function() {
			if($('input[name=mem_id]').val().trim() == ''){
		         alert("아이디를 입력해주세요.");
		         return false;
		      }
		      if($('input[name=mem_pass]').val().trim() == ''){
		         alert("비밀번호를 입력해주세요.");
		         return false;
		      }
						...
						<td>
							<table border="0" align="center" cellpadding="5"
								...
								<tr>
									<td><b>회원 가입</b></td>
									<td>
										<a href="${pageContext.request.contextPath }/user/member/memberForm.do" class="box" tabindex="3">회원 가입을 원하시면 클릭!</a>
									</td>
								</tr>
							</table>
						</td>
