...
// 리스트를 출력하는 UI에 문자열 베이스의 페이지 네이션 컨텐츠를 제공
public class RolePagingUtiles {
	...
	private int startPage;         // 페이지별 출력될 페이지 넘버의 시작번호
	private int endPage;           // 페이지별 출력될 페이지 넘버의 종료번호
	private int startCount;        // 해당 페이지의 게시글 시작 번호
	private int endCount;          // 해당 페이지의 게시글 종료 번호
	private StringBuffer pagingHTMLS;  // 페이지 네이션 컨텐츠(문자열)
	...
	public void makePagination(int blockCount) {
		if(blockCount != 0){
...
//	    </ul>
		...
		while(queryStringKeys.hasMoreElements()){
			...
			if("currentPage".intern() != queryStringKey.intern()){
				String queryStringValue = request.getParameter(queryStringKey);
		...
		if((this.currentPage - 1) == 0) {
			...
			this.pagingHTMLS.append("<li> <a href='" + requestURI + "?currentPage=" + (this.currentPage -1) + "&"+ otherParams + "'>&laquo;</a></li>");
		}
		...
		for(int i = this.startPage; i <= this.endPage; i++) {
			if(this.currentPage == i) {
				...
				this.pagingHTMLS.append("<li><a href='" + requestURI + "?currentPage=" + i + "&"+ otherParams + "'>" + i +"</a></li>");
			}
		} 
		
		// 다음
		if(this.currentPage < this.totalPage ) {
			this.pagingHTMLS.append("<li> <a href='" + requestURI + "?currentPage=" + (this.currentPage + 1) + "&"+ otherParams + "'>&raquo;</a></li>");
		}else {
